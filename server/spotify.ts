...
async function generateCoverImageInBackground(
  ...
  try {
    ...
    // We don't have a playlistId here since this is for the session, not a saved playlist yet
  ...
  app.post("/api/chat/generate", async (req: Request, res: Response) => {
    try {
      ...
      if (initialTracks.length < 24) {
        ...
        console.log(`Found ${initialTracks.length} additional tracks from general search`);

        // Get IDs of tracks we already have to avoid duplicates
        const existingIds = new Set(initialTracks.map(t => t.id));

        // We don't have any additional tracks at this point
        // The additionalTracks reference was a mistake - we already have all tracks in initialTracks
      ...
      if (initialTracks.length < 10 && user.spotifyAccessToken) {
        ...
        const additionalTracks = await spotify.searchTracks(user.spotifyAccessToken, alternativeQuery, 30, avoidExplicit);

        // Filter these tracks as well
        const validAdditionalTracks = additionalTracks.filter(track => track.duration_ms && track.duration_ms > 0);

        // Add only tracks we don't already have
        const existingIds = new Set(initialTracks.map(t => t.id));
        for (const track of validAdditionalTracks) {
          if (!existingIds.has(track.id)) {
            initialTracks.push(track);
            existingIds.add(track.id);
          }
        ...
        console.log(`Added ${validAdditionalTracks.length} additional tracks with valid duration`);
      }
      ...
      // As a last resort, if we still don't have enough tracks, use direct search
      if (tracks.length < 24 && user.spotifyAccessToken) {
        ...
        // Define alternative queries to try
      ...
      if (tracks.length < 24) {
        ...
        while (tracks.length < 24 && sortedTracks.length > 0) {
          ...
          // Use a commented property instead of a dynamic property to avoid TypeScript errors
          // We'll just log that it's a duplicate
      ...
      // Keep tracks in order of relevance
      tracks = tracks.slice(0, 24);
      console.log(`Selected ${tracks.length} tracks maintaining relevance order`);
...
The description should focus primarily on describing what's INSIDE the playlist - the actual songs, artists, and genres included. Begin with a clear statement about what listeners will find, list specific typesof songs and musical elements featured, and include 3-5 relevant hashtags at the end.`;
  ...
  app.post("/api/cover/generate", async (req: Request, res: Response) => {
    try {
      ...
      try {
        ...
        // Generate image with DALL-E and pass the playlistId to directly update the database
    ...
    try {
      ...
      if (dbPlaylist.spotifyId && user.spotifyAccessToken) {
        ...
        spotifyUrl: spotifyPlaylist.external_urls.spotify
          ...
          return;
        } catch (spotifyError) {          console.error("Error fetching Spotify playlist details:", spotifyError);
          // Continue with database-only approach
        }
      }
