...
import { db } from "../db";
import { tracks, tracksToArtists, artists, tracksToGenres, genres, albums, trackPlatformIds } from "@shared/schema";
...
async function getRandomTracks(limit: number): Promise<any[]> {
  const result = await db
    .select({
      ...
      album_cover: albums.coverImage,
      spotify_id: sql<string>`(
        SELECT platform_id 
        FROM track_platform_ids 
        WHERE track_id = ${tracks.id} AND platform = 'spotify' 
        LIMIT 1
      )`,
      spotify_url: sql<string>`(
        SELECT platform_url 
        FROM track_platform_ids 
        WHERE track_id = ${tracks.id} AND platform = 'spotify' 
        LIMIT 1
    ...
    .groupBy(tracks.id, albums.title, albums.coverImage)
    .orderBy(sql`RANDOM()`)
    .limit(limit);
